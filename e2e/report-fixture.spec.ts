...
/**
 * E-fixture smoke (V2 amendment §E). A deterministic report for `REDPROOF-BROKEN-xyz/fixture-repo` is
...
test("serves the seeded fixture report offline", async ({ page }) => {
  await page.goto("/report?repo=REDPROOF-BROKEN-xyz/fixture-repo");
  await expect(
    page.getByRole("heading", { level: 1, name: /REDPROOF-BROKEN-xyz\/fixture-repo/ }),
  ).toBeVisible();
});
